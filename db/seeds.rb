...
Snippet.create(content: "const animals = ['Dog', 'Cat', 'Sheep', 'Horse', 'John']\nconst dog = animals.filter(animal => animal === 'Dog')")
...
Snippet.create(content: "getData = async url => {\n  const response = await fetch(url)\n  return response.json()\n}")
Snippet.create(content: "createToy = async (toy, url) => {\n  const response = await fetch(url, {\n    method: 'POST',\n    headers: {\n      'Content-Type': 'application/json',\n      Accept: 'application/json'\n    },\n    body: JSON.stringify(toy)\n  })\n  return response.json()\n}")
